  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">AI-powered sales performance insights</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard label="Total Leads" value={overview?.totalLeads?.toLocaleString() ?? '—'} delta={`+${overview?.leadsToday??0} today`} icon={Users}/>
        <StatCard label="Emails Sent" value={overview?.emailsSent?.toLocaleString() ?? '—'} delta={`+${overview?.emailsToday??0} today`} icon={Mail}/>
        <StatCard label="Reply Rate" value={overview?.replyRate ? `${overview.replyRate}%` : '—'} delta="↑ this week" icon={TrendingUp}/>
        <StatCard label="Meetings Booked" value={overview?.meetingsBooked ?? '—'} delta={`+${overview?.meetingsToday??0} today`} icon={Calendar}/>
      </div>
      ...
      <div className="card p-6">
        ...
        {monthlyData.length > 0 ? (
          ...
          <div className="h-40 flex items-center justify-center text-slate-600 text-sm">No data yet — run scans to populate analytics</div>
        )}
      </div>
            ...
            <div className="space-y-4">
              {sourcesData.slice(0,5).map((s: any) => (
                <div key={s.source}>
                  ...
                  <ProgressBar value={s.count} max={sourcesData[0]?.count ?? 1} color="blue"/>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-xs text-slate-600 text-center py-6">No source data yet</p>
          )}
        </div>
      </div>
    </div>
